...
export class ThemeService {
  constructor(private preferencesService: PreferencesService) {
    ...
    window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', this.onSystemThemeUpdated.bind(this));
  ...
  private onSystemThemeUpdated(event : MediaQueryListEvent) {
    this.setTheme(event.matches ? Theme.DARK : this.preferencesService.getPreference(Preference.THEME));
  }
  ...
  setTheme(theme?: Theme) {
    let _theme: Theme = theme 
      ? theme
      : this.preferencesService.getPreference(Preference.FOLLOW_SYSTEM_THEME) === true && window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches
        ? Theme.DARK
